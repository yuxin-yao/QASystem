...
class TransformerFinder:
    def __init__(self,model_name:str = "bert-base-nli-stsb-mean-tokens",sentences:list=[]):
        ...
        if len(sentences) != 0:
            if type(sentences[0]) != str:
                self.sentences = [x.text for x in sentences]
    ...
    def batch_get_related_sentences_ranking(self,sentences:list):
        ...
        logging.info("Ranking embeddings for all the questions...")
        for i in sentence_embeddings:
            r.append(self.get_related_sentences_ranking(sentence_embedding=i))
        return r
